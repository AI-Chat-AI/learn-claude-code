...
# SkillLoader类：解析.skills目录下的.md文件，包含YAML frontmatter
...
class SkillLoader:
    ...
    def _load_all(self):
        ...
        if not self.skills_dir.exists():
            return
        # 遍历skills目录下所有.md文件（按字母顺序）
        for f in sorted(self.skills_dir.glob("*.md")):
            # 获取文件名（不含扩展名）作为skill名称
            name = f.stem
            # 读取文件内容
            text = f.read_text()
            # 解析frontmatter和正文
    ...
    # 获取所有skill的描述信息（Layer 1：系统提示词中使用的简短描述）
    ...
    # 获取指定skill的完整内容（Layer 2：在tool_result中返回完整内容）
    def get: str) ->_content(self, name str:
